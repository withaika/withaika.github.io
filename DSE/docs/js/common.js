...
var useWebIDE = ( agent.indexOf("Hudl 2")>-1 || agent.indexOf("Remix")>-1 || agent.indexOf("Runtime for Chrome")>-1 );
		...
		if( isAndroid && !useWebIDE )
		{
			...
			for( var i=0; i<list.length && list[0]!=""; i++ )
			{
				...
				if( app.IsFolder( fldr+"/"+plugName ) )
				{
					...
					var jarFiles = app.ListFolder( fldr+"/"+plugName, ".jar", 1 );
					if( jarFiles.length > 0 ) {
						var jarName = jarFiles[0];
